...
actor SearchDatabase {
    ...
    func search(
        ...
        let query = Self.normalize(rawQuery.trimmingCharacters(in: .whitespacesAndNewlines))
        let sql: String
        if query.isEmpty {
            sql = """
                ...
                WHERE i.kind = 2
                ORDER BY COALESCE(u.last_launched, 0) DESC, COALESCE(u.launch_count, 0) DESC, i.name
                LIMIT 250
                """
        } else {
    ...
    func browseLargeFiles(filter rawFilter: String, limit: Int = 100) throws -> [SearchResult] {
        try browseFiles(
            filter: rawFilter,
    ...
    func browseRecentFiles(
        ...
        let candidateLimit = preferUserDirectories ? max(limit * 10, 1_000) : limit
    ...
    private func browseFiles(
        ...
        let filterClause = filter.isEmpty ? "" : "AND i.normalized_name LIKE ? ESCAPE '\\'"
